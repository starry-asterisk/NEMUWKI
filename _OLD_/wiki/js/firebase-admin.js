...
fb.search = {
    updateAllKeywords: async () => {
        let { next } = fb.post.list();
        let arr = [];
        do {
            arr = await next();
            for (let doc of arr) {
                ...
                keyword_data.updated_timestamp = updated_timestamp;
                keyword_data.board_name_arr = board_name_arr;
                keyword_data.title_arr = title_arr;

                await firebase.search.set(doc.id, keyword_data);
            }
        } while (arr.length > 0)
    },
    set: async (id, data) => {
